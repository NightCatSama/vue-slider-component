<template>
    <div ref="wrap"
         :class="['vue-slider-wrap', flowDirection, disabledClass, { 'vue-slider-has-label': piecewiseLabel }]"
        ...
        <div ref="elem" class="vue-slider" :style="[elemStyles, bgStyle]">
            <template v-if="isMoblie">
                <template v-if="isRange">
                    <div ref="dot0" :class="[tooltipStatus, 'vue-slider-dot']" :style="[sliderStyles[0], dotStyles]"
                         @touchstart="moveStart(0)">
                        <span :class="['vue-slider-tooltip-' + tooltipDirection[0], 'vue-slider-tooltip']"
                              :style="tooltipStyles[0]">{{ formatter ? formatting(val[0]) : val[0] }}</span>
                    </div>
                    <div ref="dot1" :class="[tooltipStatus, 'vue-slider-dot']" :style="[sliderStyles[1], dotStyles]"
                         @touchstart="moveStart(1)">
                        <span :class="['vue-slider-tooltip-' + tooltipDirection[1], 'vue-slider-tooltip']"
                              :style="tooltipStyles[1]">{{ formatter ? formatting(val[1]) : val[1] }}</span>
                    </div>
                </template>
                <template v-else>
                    <div ref="dot" :class="[tooltipStatus, 'vue-slider-dot']" :style="[sliderStyles, dotStyles]"
                         @touchstart="moveStart">
                        <span :class="['vue-slider-tooltip-' + tooltipDirection, 'vue-slider-tooltip']"
            ...
            <template v-if="piecewise">
                <ul class="vue-slider-piecewise">
                    <li v-for="(piecewiseObj, index) in piecewiseDotWrap"
                        ...
                        <span class="vue-slider-piecewise-dot"
                              :style="[piecewiseStyle, piecewiseObj.inRange ? piecewiseActiveStyle : null]"></span>
                        <span v-if="piecewiseLabel" class="vue-slider-piecewise-label"
                              :style="[labelStyle, piecewiseObj.inRange ? labelActiveStyle : null]">{{ piecewiseObj.label
                            }}</span>
                    </li>
                </ul>
            </template>
